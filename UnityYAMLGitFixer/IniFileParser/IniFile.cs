...
namespace Softcircuits.IniFileParser
{
    ...
    {
        // Fields must be initialized before use in the constructor.
        ...
        {
            // 1. Initialize StringComparer first.
            StringComparer = comparer ?? StringComparer.CurrentCultureIgnoreCase;

            // 2. Initialize Sections, passing the now-initialized StringComparer.
            Sections = new Dictionary<string, IniSection>(StringComparer);

            // 3. Initialize other fields.
            BoolOptions = boolOptions ?? new BoolOptions();

            // Traditional List initialization (no [] in C# 8)
        ...
        private void ParseLine(string line, ref IniSection? section)
        {
            ...
            if (start < line.Length)
            {
                ...
                {
                    // Store comments
                    Comments.Add(line.Substring(1)); // Use Substring instead of range operator
                }
                else if (line[start] == '[')
                {
                    ...
                    // Use Substring instead of range operator
                    ...
                    if (name.Length > 0)
                    {
                        ...
                        {
                            // IniSection must be C# 8 compatible
                ...
                else
                {
                    ...
                    if (name.Length > 0)
                    {
                        ...
                        if (section.TryGetValue(name, out IniSetting? setting))
                        {
                            ...
                            Debug.Assert(setting != null); // Ensure setting is not null
                            setting.Value = value;
                        }
                        else
                        {
        ...
        public string? GetSetting(string section, string setting, string? defaultValue = null)
        {
            ...
            if (Sections.TryGetValue(section, out IniSection? iniSection))
            {
                ...
                {
                    Debug.Assert(iniSetting != null); // Added for null-safety confidence
        ...
        {
            return int.TryParse(GetSetting(section, setting), out int value) ? value : defaultValue;
        }
        ...
        {
            // BoolOptions.TryParse is assumed to be an existing method
        ...
        public IEnumerable<IniSetting> GetSectionSettings(string section)
        {
            ...
            // Use 'new List<IniSetting>()' instead of '[]' in C# 8
        ...
        public void SetSetting(string section, string setting, string value)
        {
            ...
            if (!iniSection.TryGetValue(setting, out IniSetting? iniSetting))
            {
                // Explicitly use IniSetting constructor/initializer
        ...
        /// <param name="setting">The INI setting name.</param>
        /// <param name="value">The value of the INI file setting.</param>
        public void SetSetting(string section, string setting, DateTime value) =>
            SetSetting(section, setting, value.ToString(DateTimeFormat));
